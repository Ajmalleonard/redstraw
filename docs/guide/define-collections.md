...
Content collections are the best way to manage and author content in content-first applications. RedStraw helps you organize and validate your contents, and provides type-safety through automatic type generations.
...
RedStraw uses [fast-glob](https://github.com/mrmlnc/fast-glob) to find content files, so you can use any glob pattern supported by fast-glob.

By default, RedStraw will ignore files and directories that start with `_` or `.`.
...
RedStraw uses [Zod](https://zod.dev) to validate the content items in a collection. The `schema` option is used to define the Zod schema used to validate the content items in the collection.

To use Zod in RedStraw, import the `z` utility from `'RedStraw'`. This is a re-export of Zod's `z` object, and it supports all of the features of Zod. See [Zod's Docs](https://zod.dev) for a complete documentation on how Zod works and what features are available.

```js
import { z } from 'RedStraw'
...
For more complex schemas, I recommend that you use [RedStraw extended schemas `s`](RedStraw-schemas.md):
...
For more information about RedStraw extended field schema, see [RedStraw Schemas](RedStraw-schemas.md).
...
the type of `meta` is `ZodMeta`, which extends [`RedStrawFile`](../reference/types.md#RedStrawfile). for more information, see [Custom Schema](custom-schema.md).

## Content Body

RedStraw's built-in loader keeps content's raw body in `meta.content`, and the plain text body in `meta.plain`.
...
In addition to validating the content items in a collection, RedStraw can also process the content body using [Unified](https://unifiedjs.com).
...
- [`s.markdown(options)`](RedStraw-schemas.md#s-markdown-options)
- [`s.mdx(options)`](RedStraw-schemas.md#s-mdx-options)

### Metadata

RedStraw can extract metadata from content files. This is useful for adding computed fields to the content items in a collection.
...
- [`s.metadata()`](RedStraw-schemas.md#s-metadata)

### Excerpt

RedStraw can extract excerpt from content files. This is useful for adding computed fields to the content items in a collection.
...
- [`s.excerpt(options)`](RedStraw-schemas.md#s-excerpt)
