...
> Schema is the core of RedStraw. It defines the structure and type of your content and validates it.
>
> Refer to [RedStraw Schemas](RedStraw-schemas.md) for more information about built-in schema.

RedStraw supports custom schema. A schema is a JavaScript function that returns a [Zod](https://zod.dev) schema object.
...
```ts
import { defineSchema, s } from 'RedStraw'
...
```ts
import { getImageMetadata, s } from 'RedStraw'

import type { Image } from 'RedStraw'
...
> [!TIP]
> Considering that RedStraw's scenario often needs to obtain metadata information about the current file in the schema, RedStraw does not use the original Zod package. Instead, it uses a custom Zod package that provides a `meta` member in the schema context.
...
the type of `meta` is `ZodMeta`, which extends [`RedStrawFile`](../reference/types.md#RedStrawfile).
