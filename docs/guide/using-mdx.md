...
RedStraw supports MDX out of the box. You can use MDX to write your content, and RedStraw will automatically render it for you.

> [!TIP] Some examples that may help you:
>
> - [examples/nextjs](https://github.com/zce/RedStraw/tree/main/examples/nextjs) - A Next.js and MDX example.
> - [zce/taxonomy](https://github.com/zce/taxonomy) - A fork of [shadcn-ui/taxonomy](https://github.com/shadcn-ui/taxonomy) using RedStraw.
...
Run `RedStraw build` and you will get the following data structure:
...
By default, RedStraw will compile the MDX content into a function-body string, which can be used to render the content in your application.
...
// parse the RedStraw generated MDX code into a React component function
...
You don't need to, since RedStraw's `s.mdx()` schema does not bundle those components at build time. There is no need to construct a import tree. This can help reduce output size for your contents.
...
If RedStraw uses a bundler to compile your MDX, the `Callout` component will be bundled into each MDX file, which will cause a lot of redundancy in the output code.
...
const compileMdx = async (source: string, path: string, options: CompileOptions): Promise<string> => {
  ...
  const bundled = await build({
    ...
    platform: 'neutral',
    format: 'esm',
    globalName: 'RedStraw_MDX_COMPONENT',
    treeShaking: true,
    jsx: 'automatic',
    minify: true,
    plugins: [
  ...
  return bundled.outputFiles[0].text.replace('var RedStraw_MDX_COMPONENT=', 'return ')
}
...
Then, you can use the custom schema in your `RedStraw.config.js`:
