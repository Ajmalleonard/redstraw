...
To use Zod in RedStraw, import the `z` utility from `'RedStraw'`. This is a re-export of the Zod library, and it supports all of the features of Zod.
...
In addition, RedStraw has extended Zod schemas, added some commonly used features when building content models, you can import `s` from `'RedStraw'` to use these extended schemas.
