...
RedStraw builds your contents into JSON file, and generates type inference for TypeScript, and you can use the output data in your application with confidence.
...
in `.RedStraw` directory, RedStraw generates the output files for each collection, and the `index.js` and `index.d.ts` for your application to use.
...
```js [index.d.ts]
import type __vc from '../RedStraw.config.js'
...
Because each user's scenario is different, RedStraw is framework-agnostic and does not want to dictate the structure of the user's content or how to use the output it generates. Therefore, RedStraw does not have built-in APIs related to data access.
...
```ts [app/content.ts]
import { authors, posts } from '../.RedStraw'

import type { Author, Post } from '../.RedStraw'
