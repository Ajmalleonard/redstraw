...
When running `RedStraw` from the command line, RedStraw will automatically try to resolve a config file named `RedStraw.config.js` inside project root (other JS and TS extensions are also supported).

## RedStraw Config File

RedStraw uses `RedStraw.config.js` as the config file. You can create it in the root directory of your project.
...
For better typing, RedStraw provides a `defineConfig` identity function to define the config file type.
...
In addition, RedStraw also provides a `UserConfig` type to describe the config file type.

```js
/** @type {import('RedStraw').UserConfig} */
export default {
  // ...
}
```
...
```ts
import type { UserConfig } from 'RedStraw'
...
- Type: `Schema`, See [Schema](../guide/RedStraw-schemas.md) for more information.
...
The file loaders. You can use it to load files that are not supported by RedStraw. For more information, see [Custom Loaders](../guide/custom-loader.md).
...
export default defineConfig({
  ...
  prepare: (data, context) => {
    ...
    // config is resolved from `RedStraw.config.js` with default values
