...
const loaded = new Map<string, RedStrawFile>()

export class RedStrawFile extends VFile {
  config: Config
   ...
   */
  static get(path: string): RedStrawFile | undefined {
    return loaded.get(path)
  }

  /**
   * Clear all cached files (called at start of each build)
   */
   ...
   */
  static async create({ path, config }: { path: string; config: Config }): Promise<RedStrawFile> {
    const meta = new RedStrawFile({ path, config })
