...
import { createAssetsMap } from './assets'
import { createEmittedMap } from './output'
...
const loadConfig = async (path: string): Promise<[UserConfig, string[]]> => {
  ...
  // Write the compiled config alongside the project's node_modules so that
  // Node ESM bare-specifier resolution (e.g. 'rehype-pretty-code') finds the
  // packages installed in the user's project. tmpdir does not work for this.
  const projectDir = dirname(path)
  const hash = createHash('md5').update(path).digest('hex').slice(0, 8)
  const outfile = join(projectDir, 'node_modules', '.redstraw', `config.${hash}.mjs`)
...
export const resolveConfig = async (path?: string, options: { strict?: boolean; clean?: boolean } = {}): Promise<Config> => {
  ...
  return {
    ...
    output: {
      data: resolve(cwd, loadedConfig.output?.data ?? '.redstraw'),
    ...
    assetsMap: createAssetsMap(),
    emittedMap: createEmittedMap()
  }
}
