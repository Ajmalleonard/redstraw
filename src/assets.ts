...
export const createAssetsMap = (): Map<string, string> => new Map()
...
// In-memory cache for image metadata, keyed by buffer hash (survives watch rebuilds)
const imageMetaCache = new Map<string, Omit<Image, 'src'>>()
...
export const getImageMetadata = async (buffer: Buffer): Promise<Omit<Image, 'src'> | undefined> => {
  ...
  const cached = imageMetaCache.get(hash)
  ...
  const result = { height, width, blurDataURL, blurWidth, blurHeight }
  imageMetaCache.set(hash, result)
  return result
}
...
export const processAsset = async <T extends true | undefined = undefined>(
  ...
  baseUrl: string,
  isImage?: T,
  assetsMap: Map<string, string> = createAssetsMap()
  ...
  assetsMap.set(name, path) // write to assets map waiting for copy
...
export type CopyLinkedFilesOptions = Omit<Output, 'data' | 'clean'> & { assetsMap?: Map<string, string> }
...
export const rehypeCopyLinkedFiles = (options: CopyLinkedFilesOptions) => async (tree: Hast, file: VFile) => {
  const assetsMap = options.assetsMap ?? createAssetsMap()
  ...
  await Promise.all(
    Array.from(links.entries()).map(async ([url, elements]) => {
      const publicUrl = await processAsset(url, file.path, options.name, options.base, undefined, assetsMap)
