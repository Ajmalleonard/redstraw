...
export const mdx = (options: MdxOptions = {}) =>
  custom<string | undefined>(i => i === undefined || typeof i === 'string').transform<string>(async (value, { meta, addIssue }) => {
    ...
    if (copyLinkedFiles) remarkPlugins.push([remarkCopyLinkedFiles, { ...output, assetsMap: meta.config.assetsMap }]) // copy linked files to public path and replace their urls with public urls
