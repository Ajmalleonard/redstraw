...
export const markdown = (options: MarkdownOptions = {}) =>
  custom<string | undefined>(i => i === undefined || typeof i === 'string').transform<string>(async (value, { meta, addIssue }) => {
    ...
    if (copyLinkedFiles) rehypePlugins.push([rehypeCopyLinkedFiles, { ...output, assetsMap: meta.config.assetsMap }]) // copy linked files to public path and replace their urls with public urls
