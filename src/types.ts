...
export interface Config extends Readonly<UserConfig> {
   ...
   * memory level cache is enough for RedStraw. and it's easy & efficient.
  ...
  /**
   * Assets map for current build (scoped per build invocation)
   */
  readonly assetsMap: Map<string, string>
  /**
   * Emitted content map for current build (scoped per build invocation)
   */
  readonly emittedMap: Map<string, string>
}
